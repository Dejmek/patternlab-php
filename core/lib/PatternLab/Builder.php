...
class Builder {
	...
	protected function renderPattern($f, $p) {
		...
		return array($pattern, $escaped, $d);
	}
	...
	private function generatePatternFile($f, $p, $path, $state) {
		...
		list($rf, $e, $d) = $this->renderPattern($f, $p);
		...
		if (isset($d['patternSpecific'][$p]) && isset($d['patternSpecific'][$p]['data'])) {
			file_put_contents(__DIR__ . $this->pp . $path . "/" . $path . ".json", json_encode($d['patternSpecific'][$p]['data'], JSON_PRETTY_PRINT));
		}
